...
| TRIGA-11 | **Rig proof:** lick detection runs on the real pilot driven entirely by a UI-assigned action list, with **no Python callback registered for `TOUCH_INT`** — `learning_cage` no longer assigns `self.triggers['TOUCH_INT']`. The `detectedLick` method itself is deliberately retained as the reference implementation and a one-line rollback: unregistered it is unreachable (`learning_cage` has no `CALLABLE_METHODS`, so FDA JSON cannot invoke it either), and the same method legitimately remains in `RecordingBox` and `mics_cage_task`, which are out of scope. Registration, not existence, is the property under test. Negative case: an invalid assignment is rejected at save time, not at session start. | 24 |
...
- Trigger assignment requirements: 15 (TRIGA-01–15; TRIGA-12 capability-based detector discovery → 24-06; TRIGA-13 detector keys + TRIGA-14 variables in operand pickers + TRIGA-15 trigger_name dropdown → 24-08)
