...
| 34-06-T1 | 34-06 | 3 | **SDK-15** (foreign-thread frame-rate send) | unit — 8 threads x 500 sends over FakeTransport; no exception, seq strictly increasing, no duplicates | `cd sdk && python3 -m pytest -q tests/test_client_integration.py` | ❌ W1 | ⬜ pending |
| 34-01-T3 | 34-01 | 1 | **SDK-14a/b** (tcp-only, no fork/SIGALRM/ipc, no import side effects) | AST hygiene, extended | `cd sdk && python3 -m pytest -q tests/test_import_hygiene.py` | ❌ W1 | ⬜ pending |
| 34-07-T1 + 34-07-T2 | 34-07 | 4 | **SDK-14c/e** (utf-8 + newline='', pathlib, case-insensitive suffix, path with a space) + **SDK-12 wide format** + `Pacer` | unit | `cd sdk && python3 -m pytest -q tests/test_replay.py` | ❌ W1 | ⬜ pending |
| 34-08-T2 | 34-08 | 5 | **SDK-14d** (ASCII-only README, examples and printed output) | contract test | `cd sdk && python3 -m pytest -q tests/test_readme_contract.py` | ❌ W5 | ⬜ pending |
| 34-09-T2 + 34-09-T3E | 34-09 | 6 | **SDK-14** (Windows: install into the DLC env, console script, spaced path, ASCII summary, Ctrl+C, rig send) | USER-RUN on the Windows vision box | N/A — checkpoint, see Manual-Only Verifications | N/A | ⬜ pending |
...
| Install on the **Windows vision box, inside the DeepLabCut environment** | SDK-01, SDK-14 | No agent on this dev host can run Windows; a fresh Linux venv proves nothing about the OS that will run DLC, nor about coexisting with pinned torch/numpy | In the DLC conda env on Windows: `python -m pip install <release-wheel-URL>`, then `python -c "import mics_link; print(mics_link.__version__)"`, then `python -m pip check`. Report: pip's full installed list, whether `pip check` flags any conflict, whether the import worked, and the **Windows Python version** (it decides whether `time.sleep` granularity is ~15.6 ms or ~1 ms). If no Windows box is available, record **UNPROVEN** — do not substitute a Linux venv |
| Windows replay + Windows rig send | SDK-14 | Console-script PATH, backslash/spaced paths, cp1252 console encoding and Ctrl+C during a blocking `zmq.recv` all fail **only** on Windows | From the Windows box: `python -m mics_link.replay --host 132.77.72.28 --port 5599 --source-id demo --file "C:\path with a space\replay_sample.csv" --mode realtime`. Report: (1) did `mics-link-replay` resolve as a console command or was `python -m` needed; (2) did the spaced backslash path work; (3) did the summary print without `UnicodeEncodeError`; (4) did the FDA cycle in ES as it did from Linux; (5) did Ctrl+C stop it cleanly or hang |
