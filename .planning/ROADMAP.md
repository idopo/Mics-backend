...
| 23 | 10/10 | Complete   | 2026-08-03 | ◐ **9/10 plans executed 2026-08-03** — plan 01 (Wave 0) landed the three ❌ contract test files from `23-VALIDATION.md` (CMP-04/12/17/19), all failing/skipping/xfailing today for the right reason; plan 02 landed the compute-lib storage substrate (CMP-04/12/19: `hardware_libs.kind`, `declared_imports` allowlist, `release()` gate, seeded `COMPUTE` hardware module + auto-provisioned pilot config); plan 03 landed the backend save-time gate + variable-usage analysis (CMP-10/11/15: hard 422s for compute ref/method/output, `validate_compute_variables` collision/reference checks, new `api/variable_scan.py` writer/reader existence analysis); plan 04 (Wave 2) landed `type:"compute"` on the Pi runtime (CMP-03/04/05/06) — single-sourced into `fda_vocabulary.py`/`mics_task.py`/`tools/validate_fda.py`, mandatory `output` enforced at build time in both languages; one blocking bug fixed in-task (`_build_state_method` didn't recognize `compute`), one bug found and deliberately left for plan 23-10 to confirm (`hot_update_fda` re-declaring a variable name likely hits a Phase-24 collision-guard bug); plan 05 (Wave 3) landed the single lib-version resolution chain (CMP-17/19: `resolve_lib_version_id` pin→toolkit_default→stable→active[beta/stable]→none replacing three independently-wrong chains in dispatch/introspection/orchestrator; `test_import` activated on the Pi round-trip, dead since Phase 09, with zero Pi-side change) — a fourth "active" rung was added beyond CONTEXT's literal chain so existing rig toolkits keep dispatching (documented deviation); plan 06 (Wave 3) landed the kind-aware Hardware Libraries GUI (CMP-12/18: `LibKind` types, an All/Hardware/Compute filter chip row with counts, kind-aware upload form with a stdlib-only `declared_imports` input, read-only kind/imports display on the detail page) — no new page, no new route, no `NAV_LINKS` change, no deviations; plan 07 (Wave 4) landed compute-aware preflight (CMP-15/17/19: step 6 no longer flags a compute module's empty config, self-heals a missing compute config row, `class_mismatch` reachable for compute modules; new step 9 `variable_never_written`; new step-6 `lib_version_unresolved`; `PREFLIGHT_ISSUE_KINDS` frozenset + reserved `compute_lib_import_failed_issue`; new `GET /api/task-definitions/{id}/variable-usage` route) — no deviations, full suite 332 passed, live-verified against real task defs and a real session/pilot preflight; plan 08 (Wave 4) landed the compute action in the FDA editor (CMP-13/14: exactly one new `compute` entry in the action-type `<select>`, gated off trigger assignments; new `ComputeActionFields.tsx` renders `[output] = [op ▾] ( [args] )` with ops grouped by lib via `<optgroup>`, and a mandatory output combobox that auto-declares a new `fdaJson.variables` entry on blur, immediately selectable as a `ConditionBuilder` operand) — `tsc`/`build` clean, no new page/route, behavioural click-through deferred to plan 23-10; plan 09 (Wave 5) landed GUI rendering for the three new preflight issue kinds plus the read-only variables inspector (CMP-14/15: `NON_CONFIG_ISSUES` set replaces the single-kind PUT-loop exclusion, new `ComputeIssueDetail.tsx` and `VariableUsagePanel.tsx`) — `tsc`/`build` clean, `App.tsx`/`Layout.tsx` diffs empty, behavioural sign-off deferred to plan 23-10; plan 10 remains |
