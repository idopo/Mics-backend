...
- Phase 38 added (2026-09-02): **live camera source + annotated live view for DLC-driven MICS tasks.** Phase 35's rig session proved the whole chain on a prerecorded FILE (runs 587/588, `35-HARDWARE-VALIDATION.md`, commit `8d724b0`); everything downstream of the frame source is proven and unchanged. What is missing is the frame source and any way to see what the model sees. Six concrete gaps found in that session (§3a-§3f): (1) `live.py:229` passes `--video` to `cv2.VideoCapture` as a plain string, so a local camera index cannot be opened (RTSP/HTTP URLs already work); (2) `--fps` exists because a video FILE runs unpaced — a camera paces itself via blocking `cap.read()`, so the flag double-throttles, and cameras often misreport `CAP_PROP_FPS` so the fallback is not trustworthy either; (3) `behind_count` must become a measured pass/fail criterion, because with a camera falling behind silently drops frames and staleness follows, whereas with a file it merely takes longer; (4) no viewer exists, and the vision-box env has `opencv-python-headless` (no `imshow`) which must NOT be swapped for `opencv-python` — same `cv2` module, they collide — so the viewer must render in-notebook via `IPython.display` and `DLCLive(display=False)` stays pinned (D-47); (5) the frame loop never terminates on a camera, since `_frames()` stops only when `cap.read()` fails. Also must be **model-agnostic**: the pose row order is NOT discoverable from the DLC-Live runner, so every new model needs the probe → `--pose-order` → regenerate loop as a documented path. **Sequencing: must land BEFORE Phase 37**, which extracts `dlc_link/` into a client repo — doing this after the extraction means doing it twice or in a repo the backend no longer owns. Depends on Phase 35. **Carried constraint, not to be re-litigated here:** DLC-Live reads only `get_predictions(outputs)["bodypart"]["poses"]` (`runner.py:211`) and discards the `unique_bodyparts` head, so LEDs, arena corners and mic markers are unreachable live regardless of `single_animal` — fixing that is separate work. Driver: the user has a camera being set up and wants to run this the week of 2026-09-08. **Planned 2026-09-02 — six plans exist (38-01..38-06, waves 1-5).**

- Phase 38 camera identified (2026-09-06): the sixth gap (§3f, "where the camera physically is") is **closed, with the answer that falsifies the phase's premise**. The acquisition software on the lab computer reports `DMK 33GP1300 BR2_UP 5810436`, live 30 fps — The Imaging Source, **monochrome**, **GigE Vision**, serial `5810436`. That is D-65 topology **T5**: `cv2.VideoCapture` cannot open it by any `--source` value, and because GVCP/GVSP are UDP with broadcast discovery, the T3 TCP port-forward escape hatch does not exist either. **D-75** (new) splits T5 rather than treating it as one dead end: **T5a** — The Imaging Source ships a Windows DirectShow wrapper, so the camera may already present as an ordinary capture device and the T4 MJPEG relay works with **no code change at all**; or **T5b** — no wrapper, and a GenICam capture shim behind plan 38-01's injected-capture seam is required, which D-75 declares separate work and which now has its own requirement, **CAM-17**. **The shim is built on the VENDOR's Windows SDK (The Imaging Source `imagingcontrol4`), not aravis** — corrected 2026-09-06, the vision box is Windows and aravis is a Linux/GObject stack; and before any shim is written, T6 plus the vendor driver installed on the vision box should be probed, because the camera may enumerate as an ordinary DirectShow device there and collapse T6 to T1 with no new code at all. D-75 also adds **T6**: move the camera's Ethernet to the vision box, which GigE makes possible and USB never did, and which removes the lab computer, the relay and an unsupervised always-on process from the frame path. **The discriminator is one unrun command — `ffmpeg -list_devices true -f dshow -i dummy`, on the LAB COMPUTER — and per D-67 and D-75 it should be run BEFORE wave 1**, not in wave 3 where plan 38-04 sits; it writes nothing, needs no wheel, and depends on no other plan. **So phase 38 now has a legitimate termination branch:** on T5b, plan 38-04 stops at its own step 1a-0, CAM-01/03/05/07/08 are recorded NOT EXERCISED, and 38-06 (offline, no camera, no Pi) becomes the phase's only deliverable — its `depends_on: 38-04` edge is file ownership, not a runtime fact, and must not block it. Two device facts now bind the code on the T5b path: frames are **Mono8** and need a 3-channel expansion before DLC and before the viewer's BGR draw calls; and the shim must not go through `cv2.CAP_GSTREAMER` (the headless wheel has no GStreamer; a second `cv2` install is the clobber D-58 forbids). Exclusive access is survivable: GigE Vision defines **multicast monitor** clients, so a recorder that must keep running can be attached to read-only. Registry hygiene fixed in the same pass: plan 38-06's decision set was renumbered **D-65..D-74 → D-76..D-85** and its threats **T-38-40..51 → T-38-60..71**, because both ranges were already occupied (by the topology decisions and by plan 38-05's threats); `38-DECISIONS.md` now ends with a reserved-range table. Docs updated: `38-DECISIONS.md`, `38-CONTEXT.md` §3f, `38-01`, `38-03`, `38-04`, `38-05`, `38-06`, `REQUIREMENTS.md` (CAM-16 rewritten, CAM-17 added), `ROADMAP.md`. **Next action: run the dshow probe on the lab computer.**
