#!/usr/bin/env python3
"""Interactive HIT / false-alarm trajectory dashboard (Gili).

Builds ONE self-contained offline HTML per task (plotly embedded, no server). In
the browser you can pick ANY metric for the X and Y axis, restrict to a subset of
each mouse's sessions, toggle/isolate mice, and hover a point for its numbers.

HIT / FA are defined by LICK TIMING relative to the ITI (same boundary used in
generalization_analysis.py). A trial's response window is [0, iti_start); the ITI
is [iti_start, iti_end]. Using lick BOUTS (a run of licks; a gap > LICK_BOUT_GAP
ends one):
...
HIT and FA are scored INDEPENDENTLY, so every trial contributes 0, 1, or 2 to the
HIT+FA total (0 = neither, 1 = one of them, 2 = both). Then, per session:

    accuracy      = HIT trials / (HIT trials + FA trials)   (default Y)
    participation = % of trials with ANY nose poke          (default X)

Other selectable metrics (all per session, per mouse) are listed in METRICS.

Usage:
    python3 poke_lick_dashboard.py --task appetitive
    python3 poke_lick_dashboard.py --task both
"""
...
import trial_history_analysis as TH  # loaders + task labels
import learning_trajectories_analysis as LT  # lick-bout + colour/order helpers

# (key, human label). The first two are the default axes (participation vs accuracy).
METRICS = [
    ["accuracy", "accuracy = HITs / (HITs + FAs)  (%)"],
    ["participation", "participation = % trials with any nose poke  (%)"],
    ...
    ["offcue_pokes", "pokes outside response window / trial (pre-cue or in ITI)"],
    ["latency", "latency to engage = median cue→first response-window poke  (s)"],
    ["n_trials", "session length (n trials)"],
]

# Learning-type quadrants (same thresholds as learner_criterion_analysis:
# ENGAGE_THRESH=50 participation, ACC_STRONG=70 competence). Each corner ->
# (label, colour). Splits are user-adjustable in the UI; defaults match the map.
QUADRANTS = {
    "x_thresh": 50.0,
    "y_thresh": 70.0,
    "corners": {
...
def session_row(record: dict) -> dict:
    """All dashboard metrics for one session, from its raw trials (lick-timing HIT/FA).

    A trial's response window is [0, iti_start); the ITI is [iti_start, iti_end].
    Using lick BOUTS (a run of licks; a gap > LICK_BOUT_GAP ends one):
        HIT (per trial) : a lick-bout onset BEFORE the ITI starts.
        FA  (per trial) : a lick-bout onset DURING the ITI.
    Scored independently, so each trial contributes 0, 1, or 2 to HIT+FA.
    """
    trials = record["trials"]
    n = len(trials)
    n_hit = n_fa = n_anypoke = n_oncue = n_reward = offcue_total = 0
    ...
    for t in trials:
        ...
        bouts = LT._lick_bout_onsets(t["licks"])
...
def write_dashboard(records: list[dict], out_dir: str) -> list[str]:
    ...
    for task, by_mouse in grouping.items():
        ...
        path = os.path.join(out_dir, f"poke_lick_trajectory_dashboard{suffix}.html")
...
def main() -> int:
    ...
    out_dir = str(_RESULTS_ROOT / "poke_lick_trajectory" / _TASK_AREA[args.task])
    paths = write_dashboard(records, out_dir)
    n_sess = len(records)
    print(f"\nWrote {len(paths)} dashboard(s) under '{out_dir}/' ({n_sess} sessions):")
    for p in paths:
        print(f"  {p}")
    return 0
...
<html><head><meta charset="utf-8"><title>__TITLE__ — poke→lick HIT/FA dashboard</title>
<style>
...
<div id="controls">
  ...
  <label><input id="quad" type="checkbox" checked> learning-type areas</label>
  <label id="qxwrap">x split <input id="qx" type="number" style="width:4em"></label>
  <label id="qywrap">y split <input id="qy" type="number" style="width:4em"></label>
...
// 4 learning-type areas: faint shaded quadrants + dashed split lines (shapes)
// and a label per corner (annotations), split at the x/y thresholds.
...
function redraw(){
  ...
  DATA.mice.forEach(m=>{
    ...
    traces.push({
      ...
      hovertemplate:'<b>'+m+'</b> session %{customdata[0]} (#%{customdata[1]})<br>'+
        ...
        'HIT trials %{customdata[2]} · FA trials %{customdata[3]} · trials %{customdata[4]}<extra></extra>'
    });
  ...
  const layout={
    title:{text:DATA.task_label+' — poke→lick HIT / false-alarm trajectory'+
  ...
  if(showQuad && xs.length){
    // fix the axes to the data span (incl. the split lines) so the quadrant
