...
| DVK-03 | **Derived keys appear in the view-operand picker.** `ConditionBuilder.tsx` builds `viewOpts` from `semantic_hardware` + hardware-module names + flags (lines 64-67) and renders a `<select>` whenever that list is non-empty (79-87), so `LICKER0` is currently *unreachable by typing*. Derived detector keys join that list — labelled/grouped as detector channels so `MPR121` (the device) reads differently from `LICKER0` (a channel) — everywhere the component is used: transition conditions, `ConditionGroupsEditor`, and state-body operands. | 25 |
