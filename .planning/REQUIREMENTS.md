...
| DVK-09 | **Detector channels are declared, not assumed to start at zero.** Found on hardware 2026-07-27 (runs 480/481, `24-HARDWARE-VALIDATION.md` §2b Finding A): the rig's four spouts are wired to MPR121 channels **1–4**, but `check_for_detectors` builds `f"{device_name}{i}" for i in range(num_detectors)`, so `num_detectors: 4` creates `LICKER0…LICKER3` — `LICKER0` can never fire and **channel 4's writes are silently discarded** (9 real events lost, no exception, no event). A tracker's name **must** equal its hardware channel index, because the key is `{device_name}{pin_number}` where `pin_number` is the raw index `detect_change` returns — so renumbering is not an option and the range itself must be declarable. The config gains a channel declaration (`first_channel` + count, or an explicit channel list if non-contiguous wiring is real — **decide during planning**, the rig only evidences contiguous 1–4), defaulting to today's `range(0, n)` so every existing config is unchanged. The single DVK-01 derivation helper and `check_for_detectors` consume the same declaration, so the Pi's trackers and the editor's offered keys cannot disagree. | 25 |
...
- Detector-derived view key requirements: 9 (DVK-01–09, Phase 25; DVK-09 added 2026-07-27 from rig evidence — detector channels must be declared, not assumed 0-based)
