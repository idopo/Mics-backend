...
| 23 | 4/10 | In Progress|  | ◐ **2/10 plans executed 2026-08-03** — plan 01 (Wave 0) landed the three ❌ contract test files from `23-VALIDATION.md` (CMP-04/12/17/19), all failing/skipping/xfailing today for the right reason; plan 04 (Wave 2) landed `type:"compute"` on the Pi runtime (CMP-03/04/05/06) — single-sourced into `fda_vocabulary.py`/`mics_task.py`/`tools/validate_fda.py`, mandatory `output` enforced at build time in both languages; one blocking bug fixed in-task (`_build_state_method` didn't recognize `compute`), one bug found and deliberately left for plan 23-10 to confirm (`hot_update_fda` re-declaring a variable name likely hits a Phase-24 collision-guard bug); plans 02, 03, 05–10 remain |
